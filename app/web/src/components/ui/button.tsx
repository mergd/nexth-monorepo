...
type ButtonVariant = 'default' | 'destructive' | 'outline' | 'secondary' | 'ghost' | 'link' | string
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'default', size = 'default', color, className, ...props }, ref) => {
    return (
      <RadixButton
        className={cn('cursor-pointer', variant === 'link' && 'underline-offset-4 hover:underline', className)}
        ref={ref}
